...
class Model(object):
    ...
    def run_epoch(self, trainX, trainDoW, trainM, trainL, trainXAll, valX, valDoW, valM, valL, valXAll):
        ...
        df = pd.DataFrame(columns=['epoch', 'MAE', 'RMSE', 'MAPE', 'loss'], dtype=object)
    ...
    def evaluate(self, testX, testDoW, testM, testL, testXAll):
        ...
        if not self.is_training:
            ...
            df = pd.DataFrame(columns=['steps', 'MAE', 'RMSE', 'MAPE', 'prediction time'], dtype=object)
...
def main(argv=None, Horizon=12, r_ation=0, data_name='METRLA', method_name='STGIN_2', val=1):
    '''
    :param argv:
    :return:
    '''
    ...
    para = parameter(argparse.ArgumentParser(), Horizon=Horizon, 
                     r_ation=r_ation, data_name=data_name, method_name=method_name)
    para = para.get_para()
...
if __name__ == '__main__':
    ...
    # train run
    main(Horizon=12, r_ation=0, data_name='METRLA', method_name='FI-RNNs', val=1)

    # test run
    # main(Horizon=12, r_ation=0, data_name='METRLA', method_name='FI-RNNs', val=0)
